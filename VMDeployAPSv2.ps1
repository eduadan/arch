...
#for farm install
$DBServer = 'ACUSSMDSPSQ002\inst1,1367'
...
else
{
	if ($LocalStart1)
	{
		...
		# installing windows features	
			...
			#-------media copy
				robocopy $spmedia $spmediaDestination /E
				while (!$lastexitcode)
				{
				 Start-Sleep -s 30
				}
				Write-Host "end of media copy "
				
			#-------------------
			
			set-itemproperty $RunOnceKey "installSPBorrar" 'C:\Windows\System32\WindowsPowerShell\v1.0\Powershell.exe -executionPolicy Unrestricted -File C:\Packages\Plugins\Microsoft.Compute.CustomScriptExtension\1.8\Downloads\0\my2016Install.ps1 -LocalStart3'
	...
	if ($LocalStart3)
	{
		...
		Write-Host -ForegroundColor Cyan Get-Date "  - Running Prerequisite Installer (offline mode)... DO NOT CLOSE THIS WINDOW" -NoNewline  
		...
		set-itemproperty $RunOnceKey "installSPBorrar" 'C:\Windows\System32\WindowsPowerShell\v1.0\Powershell.exe -executionPolicy Unrestricted -File C:\Packages\Plugins\Microsoft.Compute.CustomScriptExtension\1.8\Downloads\0\my2016Install.ps1 -LocalStart4'
		Start-Sleep -s 5
	...
	if ($LocalStart4)
	{
		...
		set-itemproperty $RunOnceKey "installSPBorrar" 'C:\Windows\System32\WindowsPowerShell\v1.0\Powershell.exe -executionPolicy Unrestricted -File C:\Packages\Plugins\Microsoft.Compute.CustomScriptExtension\1.8\Downloads\0\my2016Install.ps1 -LocalStart5'
		Start-Sleep -s 5
	...
	if ($LocalStart5)
	{
		...
		Start-Process "$installFolder\setup.exe" -ArgumentList "/config `"$installFolder\$xmlInstallConfigFile`"" -wait 
		Start-Sleep -s 300		
		
		
		#-------language copy
		robocopy $langPackSourceFolder $langPackFolder /E
		while (!$lastexitcode)
		{
		 Start-Sleep -s 30
		}
	...
	#-------------------
		...
		set-itemproperty $RunOnceKey "installSPBorrar" 'C:\Windows\System32\WindowsPowerShell\v1.0\Powershell.exe -executionPolicy Unrestricted -File C:\Packages\Plugins\Microsoft.Compute.CustomScriptExtension\1.8\Downloads\0\my2016Install.ps1 -LocalStart6'
		Start-Sleep -s 5
